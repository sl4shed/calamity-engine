...
SDL_AppResult Input::processInput(SDL_Event *event)
{
    switch (event->type)
    {
    ...
    {
        if (event->key.repeat)
            break; // ignore key repeat events
        auto ev = std::make_unique<InputEventKey>();
        ev->window = Services::engine()->getWindow(static_cast<int>(event->key.windowID));
    ...
    case SDL_EVENT_MOUSE_MOTION:
    {
        ...
        ev->window = Services::engine()->getWindow(static_cast<int>(event->motion.windowID));
        if (!ev->window)
            break;
        const auto camera = ev->window->getActiveCamera();

        float lx, ly;
        SDL_RenderCoordinatesFromWindow(ev->window->renderer, event->motion.x, event->motion.y, &lx, &ly);
    ...
    case SDL_EVENT_MOUSE_BUTTON_DOWN:
    {
        ...
        ev->window = Services::engine()->getWindow(static_cast<int>(event->button.windowID));
        if (!ev->window)
            break;
    ...
    case SDL_EVENT_MOUSE_WHEEL:
    {
        ...
        auto ev = std::make_unique<InputEventMouseButton>();
        ev->window = Services::engine()->getWindow(static_cast<int>(event->wheel.windowID));
...
void Input::dispatch(InputEvent &event)
{
    if (event.window)
    { // TODO: is this ok?
        event.window->root->input(event);
    }
    else if (dynamic_cast<InputEventControllerButton *>(&event) || // controller events get sent out to every window
             dynamic_cast<InputEventControllerMotion *>(&event) ||
             dynamic_cast<InputEventControllerStatus *>(&event))
    {
...
std::optional<Vector2> Input::getMousePosition() const
{
    ...
    return window->getActiveCamera()->screenToWorld({lx, ly});
}
...
{
    if (!reversedControllerMap.count(device))
        return 0.0f;
...
Vector2 Input::getVector(const std::string &minX, const std::string &maxX, const std::string &minY, const std::string &maxY, float deadzone) const
{
    ...
    if (deadzone < 0)
        deadzone = 0.2f; // fallback default
    if (len <= deadzone)
        return {0, 0};
