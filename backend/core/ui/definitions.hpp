...
public:
    ...
    // Load font from file path at runtime (non-serialization overload)
    Font *loadFromPath(const std::string &path);

    template <class Archive>
    void save(Archive &ar) const
    {
        ar(CEREAL_NVP(path), CEREAL_NVP(size), CEREAL_NVP(kerning), CEREAL_NVP(hinting), CEREAL_NVP(language), CEREAL_NVP(lineSpacing), CEREAL_NVP(outline), CEREAL_NVP(SDF), CEREAL_NVP(alignment), CEREAL_NVP(style));
    }
    ...
    {
        ar(CEREAL_NVP(path), CEREAL_NVP(size), CEREAL_NVP(kerning), CEREAL_NVP(hinting), CEREAL_NVP(language), CEREAL_NVP(lineSpacing), CEREAL_NVP(outline), CEREAL_NVP(SDF), CEREAL_NVP(alignment), CEREAL_NVP(style));
        handle = TTF_OpenFont(path.c_str(), static_cast<float>(size));
