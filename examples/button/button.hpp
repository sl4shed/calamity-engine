...
public:
    ...
    ButtonNode(const std::string &name = "ButtonNode", const std::string &text = "Button Text", Vector2 size = {100, 50}) : name(name), text(text), size(size)
    {
        // Node::Node(name);
        std::shared_ptr<Shape> shape = std::make_shared<BoxShape>(size);
    ...
    {
        ar(cereal::base_class<Node>(this), CEREAL_NVP(text), CEREAL_NVP(size));
    }
